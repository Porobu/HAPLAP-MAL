...
class RNN_Relation(object):
    def __init__(self, num_classes, word_dict_size, d1_dict_size, d2_dict_size, type_dict_size, sentMax, wv,
                 w_emb_size=50, d1_emb_size=5, d2_emb_size=5, type_emb_size=5, num_filters=100, l2_reg_lambda=0.02, learning_rate=0.001):
        ...
        with tf.variable_scope('rnn_max'):
            ...
            pooled = tf.nn.max_pool(h_rnn, ksize=[1, sentMax, 1, 1], strides=[1, 1, 1, 1], padding='VALID',
                                    name="pool")  # Nx1x200x1
    ...
    def test_step_alpha(self, W_batch, Sent_len, d1_batch, d2_batch, t_batch, y_batch):
        ...
        step, loss, accuracy, predictions, alpha = self.sess.run(
            [self.global_step, self.loss, self.accuracy, self.predictions, self.ret_alpha], feed_dict)
